/*
  ...
  Linux video capture routines base on V4L2
 ...
 */
static int opened_camid = 0;
struct capturedev *camdev_list[MAX_OPENED_CAM] = { NULL, };

/* Start stream frame capture, open video stream */
static void stream_on(struct capturedev *camdev)
{
...
static void stream_off(struct capturedev *camdev)
{
 ...
 */
static struct framedata *stream_capture(struct capturedev *camdev)
{
	int ret;
	int deqbuf_id;
...
#if 0
	pr_debug("Dequeued frame buffer id : %d-%d@%p\n", deqbuf_id,
			camdev->framemem[deqbuf_id].length,
			camdev->framemem[deqbuf_id].base);
	pr_debug("Queued frame buffer id   : %d-%d@%p.\n", buf.index,
			camdev->framemem[buf.index].length,
			camdev->framemem[buf.index].base);
#endif
...
static void show_imagfmtinfo(struct v4l2_format *imagefmt)
{
...
static void show_streamparm(struct v4l2_streamparm *stream_parm)
{
 ...
 */
static void pixformt_setting(int vidsize, uint32_t fourcc,
...
static int videodev_init(struct capturedev *camdev)
{
	int i, ret;
...
#define MMAP_PORTS (PROT_READ|PROT_WRITE)
	for(i = 0; i < bufreqs->count; i++){
		...
		capturebuf->base  = mmap(NULL, kvbuf->length,
 ...
 */
static int camdev_open(struct capturedev *camdev)
{
	int ret;
...
#if 0
		pr_debug("Video standard : %llX\n", camdev->std & V4L2_STD_PAL);
		pr_debug("Video standard : %llX\n", camdev->std & V4L2_STD_NTSC);
#endif
	}
 ...
 */
static void resources_release(struct capturedev *camdev)
{
	int i;
	struct v4l2_requestbuffers *bufreqs;
...
static void camdev_close(struct capturedev *camdev)
{
	if(camdev->fd < 0)
		return;
 ...
 */
static void camdev_imagesize(struct capturedev *camdev,
...
static int create_capturedev(int id)
{
	...
	thiscam->open = camdev_open;
	thiscam->close = camdev_close;
	thiscam->init  = videodev_init;
	thiscam->stream_on = stream_on;
	thiscam->stream_off = stream_off;
	thiscam->capture = stream_capture;
	thiscam->imagesize = camdev_imagesize;
...
int create_vidcapture(int id)
{
	...
	ret = camdev->open(camdev);
	if (ret < 0) {
		assert_failure();
		goto err;
	}

	ret = camdev->init(camdev);
	if (ret < 0) {
		assert_failure();
		goto err;
	}

	camdev->stream_on(camdev);
